...
const DEVELOPERS = {
  "712020:d9aa0306-1300-44fc-bd29-8abe306f07fe": {
    name: "Abubakar Adamu",
    email: "abubakar.adamu.ext@brightlysoftware.com",
  },
  "712020:407d6248-323b-459e-a956-57d36e03a526": {
    name: "Adinan Alhassan",
    email: "adinan.alhassan.ext@brightlysoftware.com",
  },
  "712020:f498aef4-0ecd-43b5-8627-6e8729712986": {
    name: "Abenezer Bayu",
    email: "abenezer.bayu.ext@brightlysoftware.com",
  },
  "712020:4e753b4c-292f-43c1-aec8-346aa573fc63": {
    name: "Emmy Bbaale",
    email: "emmy.bbaale.ext@siemens.com",
  },
  "712020:391f1fcb-67ae-4668-956c-0025cc785212": {
    name: "Ojobe Ekpor",
    email: "ojobe.ekpor.ext@brightlysoftware.com",
  },
  "712020:9484d75f-f78a-4c62-86d5-e3202d55e129": {
    name: "Kashish Goyal",
    email: "kashish.goyal.ext@siemens.com",
  },
  "712020:98376025-a295-45d3-8479-a6c4c617708b": {
    name: "Michael Johnson",
    email: "michael.johnson.ext@brightlysoftware.com",
  },
  "712020:5a76440b-b20f-4c76-9fe0-e08db894a301": {
    name: "Nilesh Pore",
    email: "nilesh.pore.ext@brightlysoftware.com",
  },
  "712020:9f4e93bd-1ced-489a-8034-3965e2ca3beb": {
    name: "Touqeer Shakeel",
    email: "touqeer.shakeel.ext@brightlysoftware.com",
  },
  "712020:b8298392-2a8a-4bc9-ae32-87f56ff8eb8e": {
    name: "Bhanu Teja",
    email: "bhanu.teja.ext@brightlysoftware.com",
  },
};
...
function classifyDefect(summary) {
  ...
  if (/api|server error|500|graphql|endpoint|grpc|rest/.test(s))
    return "API / Backend";
  if (
    /ui |display|mismatch|label|button|scroll|pagination|layout|icon|dropdown|modal|dialog|screen|distort|tooltip|style|css/.test(
      s,
    )
  )
    return "UI / Frontend";
...
for (let i = 0; i < allFiles.length && i < FILE_TO_DEV.length; i++) {
  ...
  for (const issue of issues) {
    ...
    if (!seenKeys.has(key)) {
      seenKeys.add(key);
      console.log(
        `  ${key}: ${status} | ${priority} | bounces=${bounces} | ${defectType}`,
      );
    }
  }
...
for (const [devId, d] of Object.entries(devData)) {
  ...
  const done = issues.filter((i) =>
    ["Done", "Closed", "Resolved", "Verified", "Fixed"].includes(i.status),
  ).length;
  const bounceVals = issues.filter((i) => i.bounces > 0).map((i) => i.bounces);
  const avgBounces = bounceVals.length
    ? (bounceVals.reduce((a, b) => a + b, 0) / bounceVals.length).toFixed(1)
    : 0;
  const rftTimes = issues
    .filter((i) => i.timeToRftDays !== null)
    .map((i) => i.timeToRftDays);
  const avgRft = rftTimes.length
    ? (rftTimes.reduce((a, b) => a + b, 0) / rftTimes.length).toFixed(1)
    : "N/A";
  ...
  const avgClose = closeTimes.length
    ...
    : "N/A";
  console.log(
    `${d.name.padEnd(25)} | total=${total.toString().padStart(3)} | done=${done.toString().padStart(3)} | avg_bounces=${avgBounces} | avg_rft=${avgRft} days | avg_close=${avgClose} days`,
  );
}

// Save metrics JSON
const outPath = path.join(__dirname, "dev_metrics.json");
